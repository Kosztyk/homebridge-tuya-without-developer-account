...
Live Tuya Device Sharing MQTT traces confirm this fan has two physical light channels. `switch_led` controls the white lamp, `colour_data` carries static RGB HSV data, and proprietary raw DP `103` reports RGB state (`off`, `mode_10`, etc.). Raw DP103 is report-only through the HA QR command endpoint: attempting `103=off` is rejected with Tuya error `2008`. `colour_switch=true` starts the rainbow/effect mode and is therefore **not** used for normal RGB ON. The plugin exposes separate White Light and RGB Light HomeKit services, turns static RGB on by re-sending the last `colour_data`, and turns it off with the writable schema command `colour_switch=false`; if that direct OFF is not confirmed, it performs one effect-on/effect-off fallback matching the Smart Life UI behavior. Incoming DP103 `off` maps to RGB OFF, while `mode_*` maps to RGB active. RGB brightness is intentionally omitted because this firmware does not offer adjustable RGB brightness.
